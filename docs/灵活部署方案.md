...
|**`monit`**|因为你已经有K8S监控了，monit下面的所有`enable`都配置成`false`。|
...
>特别说明：如果你已经有完整的K8S监控系统（满足3个前提），并且你也不需要KubeDoor去操作你的K8S（KubeDoor-Web上对K8S执行扩缩容、重启、Pod隔离/Dump等等操作）,那么你也可以完全不安装KubeDoor-agent，仅安装KubeDoor-Master即可。（监控、告警、高峰期数据的展示分析能力Master从ClickHouse与VictoriaMetrics中获取数据即可。）
